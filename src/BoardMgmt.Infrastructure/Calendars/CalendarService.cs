...
public sealed class CalendarService : ICalendarService
{
    ...
    public CalendarService(GraphServiceClient graph, IOptions<GraphOptions> opts)
    {
        _graph = graph;
        _opts = opts.Value;
    }
    ...
    public async Task<(string eventId, string? joinUrl)> CreateEventAsync(Meeting m, CancellationToken ct = default)
    {
        ...
        var created = await _graph.Users[_opts.MailboxAddress].Events.PostAsync(ev, cancellationToken: ct);
    ...
    public async Task<(bool ok, string? joinUrl)> UpdateEventAsync(Meeting m, CancellationToken ct = default)
    {
        ...
        await _graph.Users[_opts.MailboxAddress].Events[m.ExternalEventId]
            .PatchAsync(patch, cancellationToken: ct);

        // 👇 FIX: name the cancellationToken parameter
        var refreshed = await _graph.Users[_opts.MailboxAddress].Events[m.ExternalEventId]
